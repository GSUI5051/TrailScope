...
- **渲染与加载优化**  
  使用 `Path2D` 批量绘制同色剖面路径，缓存分段分析、派生注释和地图渲染数据，在不同渲染模式间共享地图坐标；Tailwind 样式采用预编译 CSS，并且只在导入 KMZ 时按需加载 JSZip。GPX 解析一次建立最终点对象形状，三维距离复用已计算的水平距离；连续选择轨迹时会忽略过期的异步读取结果并取消过期延迟绘制。图标改由本地 SVG 数据渲染，不再加载图标字体。
...
4. **交互操作**  
   - **路线地图**：缩放、平移；图源选择支持搜索、按类型分组，并记住最近使用的三个图源。点击航路点可查看详细信息。  
...
- **react-icons 5.7.0** – 图标数据来源，包含 Font Awesome 6、Lucide 与 Bootstrap Icons 三套图标；SVG 数据提取后自托管在 `js/common/icons-data.js`，由 `icons.js` 渲染为内联 SVG，离线可用
...
│   │   ├── icons-data.js       # 图标 SVG 数据（提取自 react-icons 的 fa6/lu/bi 模块）
│   │   ├── icons.js            # 图标渲染器，把 <i> 标记替换为内联 SVG
...
- 感谢 Leaflet、Tailwind CSS、react-icons 等开源库；图标数据来自 Font Awesome、Lucide 与 Bootstrap Icons。
- 感谢所有地图服务商。
