...
// ★★★ Dark-mode tile filters (Leaflet.TileLayer.ColorFilter) — keys match mapSources ★★★
const DARK_TILE_FILTERS = {
    // Satellite / Amap layers
...
// The filter is applied only to the tile layer's own container, leaving the
// track markup pane untouched. Call this after switching source or theme to
// keep the filter state in sync with the current theme + source.
...
function changeMapSource(source, skipTrackDraw = false) {
    ...
    // Keep the filter state in sync after a source switch (theme is handled on change).
    updateMapTileFilter();
